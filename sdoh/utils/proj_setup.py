...
def remove_files(path): 
    ...
    #Remove files in directory (not recursive) 
...
def create_scratch(path, root, scratch):

    # Relative path     
    relpath = os.path.relpath(path, start=root)

    # Equiv scratch path     
    fullpath = os.path.join(scratch, relpath)
...
def setup_dir(dest, root, scratch=None, clear_scratch=True):
